...
def deepFM(nums_dict, embedding_size=32, tag_history_max_len=50, dnn_hidden_units=(128, 128),
    ...
    # 对tagid_history进行处理
    tagid_lr_emb = SequencePoolingLayer(mode='sum')(tagid_lr_emb)  # (B, 1, 1)

    tagid_emb = Transformer(att_embedding_size=embedding_size/8,head_num=8,use_positional_encoding=False,supports_masking=True)\
        ([tagid_emb,tagid_emb])
    # lr
    ...
    model = tf.keras.models.Model(
        inputs=[gender, age, province, city, tagid_history], outputs=output)
    return model